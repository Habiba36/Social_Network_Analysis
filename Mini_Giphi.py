...
def upload_file():
    ...
    def upload_Btn(button_id):
        ...
        global edges_path
        if (button_id == "nodes"):
            nodes_path = filedialog.askopenfilename(initialdir="/home/rengo/Downloads/social project")

        else:
            edges_path = filedialog.askopenfilename(initialdir="/home/rengo/Downloads/social project")
    ...
    directed_button = Radiobutton(new_window, text="Directed", variable=type_of_graph, value=1,
                                  command=lambda: when_change(type_of_graph.get()))
    directed_button.pack()

    undirectd_button = Radiobutton(new_window, text="Undirected", variable=type_of_graph, value=0,
                                   command=lambda: when_change(type_of_graph.get()))
...
def girven_newman():
    ...
    def run_girven():
        ...
        number_of_communities, modularity, communities, G = ts.compute_by_girvenewman(nodes_df, edges_df,
                                                                                      type_of_graph.get(), True, n=n)
...
def louvain():
    ...
    number_of_communities, modularity, communities, G = ts.compute_by_louvain(nodes_df, edges_df, type_of_graph.get(),
                                                                              True)
...
def evaluationn(window, communities, G):
    ...
    b = ttk.Button(window, text="calc",
                   command=lambda: community_detection_evaluation(evaluation_type.get(), communities, G)).pack()
